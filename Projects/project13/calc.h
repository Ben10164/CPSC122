 ...
 */
   ...
   /*
  pre: CheckParens is called
  post: Changes the values of tmp, amount, and can change bal
 */
   void CheckIfParenValid(char ch, Stack &tmp, int &amount, bool &bal);

   /*
  pre: inFix[inFixIdx] is an int
  post: the int is pushed onto the stack after being converted into its real int
 */
   void NumberInFixConversion(int &inFixIdx, char *&tmp, int &tmpIdx);

   /*
  pre: postFix[i] is a not an alphabet char
  post: The Operation occurs
 */
   void DoOperation(char ch, int &result, int op1, int op2);

   /*
  pre: inFix[i] != '\0'
  post: inFix is converted to postFix
 */
   void ConversionToPostFix(char *tmp, char temp);
